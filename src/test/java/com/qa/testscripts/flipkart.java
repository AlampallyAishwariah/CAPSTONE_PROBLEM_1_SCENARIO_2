...
public class flipkart extends TestBase{
	...
	public void checkPosition() throws IOException {
		if(p.getFlipKartLogo().isDisplayed()) {
			...
			int logolocation=p.getFlipKartLogo().getLocation().getX();
			if(logolocation<500) {
	...
	public void checkSearch() throws InterruptedException, IOException {
		p.getSearchbar().sendKeys("iphone 14");
		Thread.sleep(2000);
		boolean search_button=p.getSearchSymbol().isEnabled();
		if(search_button) {
			Assert.assertTrue(true);
		}
		else {
